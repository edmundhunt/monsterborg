...
def PointTransformation(UTMPoint_input):
    ...
    while notDone:
        try:
            ...
            listener.waitForTransform('r2_tf/odom', 'utm', rospy.Time.now(), rospy.Duration(10.0))

            # Transforms geometry_msgs/PointStamped message to frame target_frame=odom
            TransformedPoint = listener.transformPoint('r2_tf/odom', UTMPoint_input)
            notDone = False
        except tf.Exception:
...
def move_base_goal(curr_point, next_point, last_point):
    ...
    goal.target_pose.header.frame_id = 'r2_tf/odom'
...
# ************************************ Main Program ************************************:
# Node name:
rospy.init_node('r2_gps_waypoint')
...
client = actionlib.SimpleActionClient('/r2/move_base', MoveBaseAction)
pub_PointStamped = rospy.Publisher('r2_gps_point', PointStamped, latch=True, queue_size=1)
